...
public class SpringBootScan {
    private List<String> SpringPath = new ArrayList<>(List.of(
            "api-docs",
            "actuator",
            "actuator/auditLog",
    ...
    private  String ErrorPath="aaaaaaaaa";

    public SpringBootScan(String Baseurl,HttpRequest httpRequest,MontoyaApi api){
        this.baseUrl=Baseurl;
        this.httpRequest=httpRequest;
        this.api=api;
    }
    public boolean checkSpring(){
        String errorpath=baseUrl+ErrorPath;
        HttpRequest errorRequest=httpRequest.withPath("/"+errorpath).withMethod("GET").withBody("");
        api.logging().logToOutput("errorpath "+errorRequest.url()+"\n");

        HttpResponse errorResponse=api.http().sendRequest(errorRequest).response();
        api.logging().logToOutput("test "+errorResponse.bodyToString());
        if (errorResponse.statusCode() == 200 || (errorResponse.bodyToString().contains("Whitelabel Error") || errorResponse.bodyToString().contains("Not Found")|| errorResponse.bodyToString().contains("mapping for /error"))) {
            return true;
        }
        return false;
    }

    public HashMap<HttpRequest,HttpResponse>PathScan() throws InterruptedException {
        HashMap hashMap=new HashMap<>();
        for(String path: SpringPath){
            HttpRequest request=httpRequest.withPath("/"+path).withMethod("GET").withBody("");
            HttpResponse response = api.http().sendRequest(request).response();
            api.logging().logToOutput("path "+request.url()+"\n");
