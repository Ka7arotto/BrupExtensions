...
public class CustomLogger implements BurpExtension
{
    ...
    public void initialize(MontoyaApi api)
    {
        ...
        api.extension().setName("SpringBoot-Scan");
        api.logging().logToOutput("brup start!");
        api.logging().logToError("brup error!");

        MyTableModel tableModel = new MyTableModel();

        api.userInterface().registerSuiteTab("SpringBoot-Scan", constructLoggerTab(tableModel));
        api.http().registerHttpHandler(new MyHttpHandler(tableModel,api));
    }
